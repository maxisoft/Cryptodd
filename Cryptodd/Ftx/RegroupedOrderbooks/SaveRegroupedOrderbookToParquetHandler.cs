...
public class SaveRegroupedOrderbookToParquetHandler : IRegroupedOrderbookHandler
{
        ...
        bool gzip)
    {
        ...
        groupWriter.WriteColumn(new DataColumn(marketColumn, orderbooks.Select(o => PairHasher.Hash(o.Market)).ToArray()));
        ...
        for (var i = 0; i < orderBookDeepness; i++)
        {
            groupWriter.WriteColumn(new DataColumn(dataFields[0], orderbooks.Select(orderbook => orderbook.Bids[i].Price).ToArray()));
            groupWriter.WriteColumn(new DataColumn(dataFields[1], orderbooks.Select(orderbook => orderbook.Bids[i].Size).ToArray()));
        ...
        for (var i = 0; i < orderBookDeepness; i++)
        {
            groupWriter.WriteColumn(new DataColumn(dataFields[0], orderbooks.Select(orderbook => orderbook.Asks[i].Price).ToArray()));
            groupWriter.WriteColumn(new DataColumn(dataFields[1], orderbooks.Select(orderbook => orderbook.Asks[i].Size).ToArray()));
            dataFields = dataFields[2..];
        }
