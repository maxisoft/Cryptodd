...
public class EmptyBinanceRateLimiter : IInternalBinanceRateLimiter, INoAutoRegister
{
    ...
    public ValueTask<IApiCallRegistration> WaitForSlot(Uri uri, int weight, CancellationToken cancellationToken) => ValueTask.FromResult<IApiCallRegistration>(new EmptyApiCallRegistration(){Uri = uri, Weight = weight});
